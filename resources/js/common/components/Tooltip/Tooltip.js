...
class Tooltip extends React.Component {
  ...
  handleAppear = () => {
    if (!this.state.visible) {
      ...
      this.animation = TweenMax.fromTo(this.tooltip, 0.2, { scale: 0.5, opacity: 0 }, { scale: 1, opacity: 1 });
  ...
  handleDisappear = () => {
    if (this.state.visible) {
      ...
      this.animation = TweenMax.fromTo(this.tooltip, 0.2, { scale: 1, opacity: 1 }, { scale: 0.2, opacity: 0 });
  ...
  render() {
    return (
      ...
      >
        {this.props.children}
        <StyledTooltip ref={span => (this.tooltip = span)}>{this.props.title}</StyledTooltip>
      </StyledTooltipBox>
    );
  }
}
